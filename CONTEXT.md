...
4. (선택) **관리자 권한 분리 — 클라이언트 가림까지만 구현.** `settings.managers`(조정자)에 지정된 멤버가
   '나'로 선택된 브라우저만: 미팅·설정 탭이 보이고, 시간표에서 일정을 고치고(빈 칸 추가·시간 변경·취소·삭제),
   남의 '내 시간'도 대신 입력한다. 나머지는 보기 + 본인 '내 시간'(반복 페인팅·예외)만 — `isManager()` /
   `canEditAv(mid)`가 판정하고, UI 숨김에 더해 handleClick/handleChange 앞단의 `actAllowed()`(MANAGER_ACTS)가
   이중 방어한다. 비우면 전원 조정자, 유효하지 않은 id만 남으면 전원으로 복귀(잠금 사고 방지).
   이것은 화면 가림일 뿐이며(누구나 '나'를 바꿔 뚫을 수 있다) 진짜 권한은 여전히 Firebase Auth + 규칙이 필요하다. 현재는 밴드 앱과 같은 "링크 아는 사람 전부 편집" 모델이다.
